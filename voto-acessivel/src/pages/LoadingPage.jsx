...
export default function LoadingPage() {
  ...
  const divRef = useRef(null);  // <--- adicionado
  ...
  useEffect(() => {
    ...
    // foco automático no div principal
    if (divRef.current) {
  ...
  const handleStart = () => {
    // Ativar varrimento ao clicar
    window.varrimentoAtivo = true;

    // Frase para desbloquear speech synthesis
    const utterance = new SpeechSynthesisUtterance("Bem-vindo à plataforma de voto acessível. A navegação vai começar.");
    utterance.lang = "pt-PT";
    utterance.volume = 0.5;  // volume entre 0 (mudo) e 1 (máximo)
    ...
    <div 
      ...
      onKeyDown={(e) => {
        if (e.key === "Enter" || e.key === " ") {
          e.preventDefault();  // <-- isso aqui evita scroll com espaço
          handleStart();
        }}} // Acessibilidade teclado
      aria-label="Clique para começar e ativar o leitor de voz"
    >
      <div className="logo-container">
        ...
        <div className="tipo-logo" >
          <h1 className="loading-title">Voto Acessível</h1>
          <h2 className="loading_subtitle" >— plataforma de voto acessível —</h2>
        </div>
      </div>
      
      <FullscreenButton />
      {isFullscreen}
      <button className="click-text">
        Clique para começar
      </button>
    </div>
  );
